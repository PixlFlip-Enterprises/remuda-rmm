/**
 * FF2 — cross-package wire-contract guard (CLIENT side, PowerPoint).
 *
 * Asserts this dispatcher's POWERPOINT_TOOL_EXECUTORS key set and
 * POWERPOINT_MUTATING_TOOLS set EXACTLY equal the PowerPoint slice of the shared
...
describe('PowerPoint dispatcher <-> server contract parity', () => {
  it('POWERPOINT_TOOL_EXECUTORS keys exactly equal the contract tool names', () => {
    expect(Object.keys(POWERPOINT_TOOL_EXECUTORS).sort()).toEqual(contractToolNames(HOST));
  });

  it('POWERPOINT_MUTATING_TOOLS exactly equals the contract mutating set', () => {
    expect([...POWERPOINT_MUTATING_TOOLS].sort()).toEqual(contractMutatingToolNames(HOST));
  });

  it('every executor name is a known contract tool (no client-only tools)', () => {
    const contractNames = new Set(contractToolNames(HOST));
    for (const name of Object.keys(POWERPOINT_TOOL_EXECUTORS)) {
  ...
  it('every contract tool has a backing executor (no missing tools)', () => {
    for (const name of contractToolNames(HOST)) {
      expect(POWERPOINT_TOOL_EXECUTORS[name], `contract tool '${name}' has no executor`).toBeTypeOf(
        'function',
      );
    }
  });
