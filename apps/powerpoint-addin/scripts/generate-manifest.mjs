...
const baseUrl = (argValue('--base-url') ?? env.ADDIN_BASE_URL ?? 'https://localhost:3003').replace(/\/$/, '');
...
// or Office caches collide across hosts). MUST be distinct from the Excel and Word
// add-in GUIDs — reusing one makes Office cache multiple hosts as one add-in.
const addinId = env.ADDIN_MANIFEST_ID ?? '3154ca26-421d-44a1-83e7-4c976f0a8099';
