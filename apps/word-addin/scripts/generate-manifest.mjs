...
//   node scripts/generate-manifest.mjs [--base-url https://localhost:3002] [--out manifest.xml]
...
const baseUrl = (argValue('--base-url') ?? env.ADDIN_BASE_URL ?? 'https://localhost:3002').replace(/\/$/, '');
...
// or Office caches collide across hosts). MUST be distinct from the Excel add-in's
// GUID — reusing it makes Office cache both hosts as one add-in.
const addinId = env.ADDIN_MANIFEST_ID ?? 'cf1e2379-35f2-40df-bf3d-04cc6899d7ce';
