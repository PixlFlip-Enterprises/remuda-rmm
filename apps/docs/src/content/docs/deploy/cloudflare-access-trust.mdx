...
<Aside type="caution">
  For sign-out to clear the Cloudflare Access session too, `GET /api/v1/auth/cf-access-logout` must itself be enforced by your Access application (not swallowed by a blanket `/api/*` bypass rule). See the path list in **Recommended Cloudflare Access setup** below.
</Aside>
