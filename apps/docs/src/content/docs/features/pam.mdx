...
<Aside type="tip">
  PAM never lets a request self-approve. An AI- or Helper-initiated elevation, and a UAC prompt with no matching rule, always default to **pending** -- waiting on a person -- so automation can never quietly relax its own approval.
</Aside>
...
<Aside type="caution">
  The approver is always a separate identity from the requester. A user can't approve their own elevation, and an AI agent or Helper session can't sign off on the action it proposed -- approval requires a human with the right permission, confirmed with MFA.
</Aside>
...
<Aside>
  UAC interception is **ON by default at every level**. Existing deployments keep capturing prompts with no action needed; you opt a scope *out* by assigning a policy that turns the toggle off. When capture is disabled, the agent stops surfacing new UAC elevation requests for those devices.
</Aside>
