...
In addition to row-level immutability, each row carries `prevChecksum` linking to the previous audit row in the same organization. Together with `checksum`, this produces a per-org SHA-256 hash chain. The link is **sealed at commit time** under a per-organization advisory lock, so concurrent writes from agents and users cannot fork the chain. Verifying the chain end-to-end detects insertion, deletion, or alteration of any row between two timestamps.

Retention pruning is the one supported path that removes audit rows, and it operates by **prefix-cut**: the contiguous oldest segment that has aged past the retention window is deleted, and the first surviving row becomes the new chain head. There is **no re-anchoring** — the verifier accepts a null `prevChecksum` on the earliest retained row as evidence of a pruned prefix rather than a break, so the integrity check continues to pass after old data ages out.
