...
description: Configure OIDC and SAML identity providers for organization-level single sign-on in Breeze RMM.
sidebar:
  order: 11
  label: SSO
---
...
<Aside>
  SAML 2.0 provider types are fully modeled in the database and the SSO service includes complete SAML request/response parsing, SP metadata generation, and redirect URL building. The SAML login flow through the SSO routes currently supports OIDC. SAML integration is available at the service layer and can be activated for your deployment.
</Aside>
